...
var elasticsearch = function(parent, url, index) {
  ...
  this.scrollIdFile = '/var/tmp/' + ('elasticdump/'+this.base.url+'/_scroll_id').split('/').join('_');

  if (parent.options.resume) {
    this.lastScrollId = String( fs.readFileSync(this.scrollIdFile, 'utf8') )
  }
};
// accept callback
...
elasticsearch.prototype.getData = function(limit, offset, callback) {
  ...
  if (self.lastScrollId !== null) {
    ...
    request.get(searchRequest, function requestResonse(err, response) {
      ...
      fs.writeFileSync(self.scrollIdFile, self.lastScrollId);
